...
definition "case_of_6 s u v w x y z     \<equiv> \<exists>a b c d e f. ((a+b+c+d+e+f = N) \<and> a\<ge>0 \<and> b\<ge>0 \<and> c\<ge>0 \<and> d\<ge>0 \<and> e\<ge>0 \<and> f\<ge>0
                                        \<and>(\<forall>i.(0\<le>i \<and> i<a)\<longrightarrow>ownB s i = u)
                                        \<and>(\<forall>i.(a\<le>i \<and> i<a+b)\<longrightarrow>ownB s i = v)
                                        \<and>(\<forall>i.(a+b\<le>i \<and> i<a+b+c)\<longrightarrow>ownB s i = w)
                                        \<and>(\<forall>i.(a+b+c\<le>i \<and> i<a+b+c+d)\<longrightarrow>ownB s i = x)
                                        \<and>(\<forall>i.(a+b+c+d\<le>i \<and> i<a+b+c+d+e)\<longrightarrow>ownB s i = y)
                                        \<and>(\<forall>i.(a+b+c+d+e\<le>i \<and> i<a+b+c+d+e+f)\<longrightarrow>ownB s i = z))"

definition "case_of_5 s u v w x y     \<equiv> \<exists>a b c d e. ((a+b+c+d+e = N) \<and> a\<ge>0 \<and> b\<ge>0 \<and> c\<ge>0 \<and> d\<ge>0 \<and> e\<ge>0
                                        \<and>(\<forall>i.(0\<le>i \<and> i<a)\<longrightarrow>ownB s i = u)
                                        \<and>(\<forall>i.(a\<le>i \<and> i<a+b)\<longrightarrow>ownB s i = v)
                                        \<and>(\<forall>i.(a+b\<le>i \<and> i<a+b+c)\<longrightarrow>ownB s i = w)
                                        \<and>(\<forall>i.(a+b+c\<le>i \<and> i<a+b+c+d)\<longrightarrow>ownB s i = x)
...
definition "case_comp s \<equiv> case_of_6 s Q W B R Q D \<or> 
                          case_of_5 s R Q W B D \<or>
                          case_of_5 s B R Q W B"
...
lemma trial:
  ...
  and "ownB s 3 = R"
  and "ownB s 4 = R"
shows "case_of_6 s Q W B R Q D"
  using assms apply (simp add:case_comp_def case_of_5_def case_of_6_def)
  apply(rule exI [where x=0])
  apply(rule exI [where x=1])
  apply(rule exI [where x=2])
  apply(rule exI [where x=2])
  apply(rule exI [where x=0])
  apply(rule exI [where x=0]) apply(intro conjI) 
  apply linarith
  apply auto
  apply (metis One_nat_def Suc_1 le_less_Suc_eq less_antisym)
  by (metis Suc_lessI eval_nat_numeral(2) le_less not_less_eq numeral_3_eq_3 semiring_norm(26) semiring_norm(27))
...
lemma trial2:
  ...
  and "ownB s 4 = R"
shows "case_comp s"
  using assms(1) assms(2) assms(3) assms(4) assms(5) assms(6) case_comp_def trial by auto

*)


definition "case_1 s  \<equiv> \<exists>a b c d e. (0\<le>a \<and> a\<le>b \<and> b\<le>c \<and> c\<le>d \<and> d\<le>e \<and> e\<le>N
                                        ...
                                        \<and>(\<forall>i.(d\<le>i \<and> i<e)\<longrightarrow>ownB s i = B)
                                        \<and>(\<forall>i.(e\<le>i \<and> i<N)\<longrightarrow>ownB s i = D)
                                \<and> (a>0\<longrightarrow>(e=N \<and> T s=a))
                                \<and> (e<N\<longrightarrow>(d>0 \<and> d<e \<and> T s=e)))
                                \<and> (H s=d)"

definition "case_2 s  \<equiv> \<exists>a b c d e. (0\<le>a \<and> a\<le>b \<and> b\<le>c \<and> c\<le>d \<and> d\<le>e \<and> e\<le>N 
                                        \<and>(\<forall>i.(0\<le>i \<and> i<a)\<longrightarrow>ownB s i = Q)
                                        \<and>(\<forall>i.(a\<le>i \<and> i<b)\<longrightarrow>ownB s i = W)
                                        \<and>(\<forall>i.(b\<le>i \<and> i<c)\<longrightarrow>ownB s i = B)
                                        \<and>(\<forall>i.(c\<le>i \<and> i<d)\<longrightarrow>ownB s i = R)
                                        \<and>(\<forall>i.(d\<le>i \<and> i<e)\<longrightarrow>ownB s i = Q)
                                        \<and>(\<forall>i.(e\<le>i \<and> i<N)\<longrightarrow>ownB s i = D)
                                \<and> ((e=N \<and> b>0 \<and> b<N)\<longrightarrow>c>b)
                                \<and> (e<N\<longrightarrow>(b>0 \<and> c>b \<and> T s=c)))
                                \<and> (H s=b)"
...
shows "case_2 s"
  ...
  apply(rule exI [where x=0])
  apply(rule exI [where x=1]) apply(intro conjI impI) apply blast
  apply(rule exI [where x=3]) apply(intro conjI impI) apply linarith
  apply(rule exI [where x=5]) apply(intro conjI impI) apply linarith
  apply(rule exI [where x=5]) apply(intro conjI impI) apply linarith
  apply linarith 
  apply linarith
  apply auto
  apply (metis One_nat_def Suc_1 le_refl less_2_cases not_less_eq_eq not_less_less_Suc_eq numeral_3_eq_3)
  apply(case_tac "i\<in>{3,4}")
  by auto
