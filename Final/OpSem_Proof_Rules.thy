...
apply simp
  ...
  apply(elim conjE)
  apply clarsimp 
  using assms(2) assms(3) apply auto[1]  using assms(2) avar.simps(1) 
  using assms(3) apply force
  using assms(2) avar.simps(1) apply auto[1] 
  by (smt (verit) fun_upd_same nle_le prod.sel(2) surrey_state.select_convs(2) surrey_state.select_convs(4) surrey_state.surjective surrey_state.update_convs(2) ts_oride_def tst_def update_thrView_def)
...
shows "init_val \<sigma>' x u"
  ...
  assume a1: "init_val \<sigma> x u"  and
         ...
         a5: "valid_fresh_ts \<sigma> (a, b) ts'" 
...
shows "amo \<sigma>' x v"
  ...
  apply (unfold writes_on_def, simp) 
  apply auto[1] 
  by (simp add: add_cv_def update_mods_def update_modView_def update_thrView_def)
...
shows "v = u"
  using assms
  apply(simp add: d_obs_t_def d_obs_def lastWr_def tst_def step_def RdX_def visible_writes_def read_trans_def rev_app_def)
  apply(simp add: update_thrView_def)
  apply(unfold  writes_on_def)[1]
  apply(elim exE conjE)
  by (metis assms(2) assms(3) d_obs_RdX_pres d_obs_p_obs_agree p_obs_RdX_intro wfs_preserved)
