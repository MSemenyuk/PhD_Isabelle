...
preCond ms' ps' \<sigma>' (pc ms' t) t"
  ...
  apply(case_tac "pc ms t", simp_all add:abbr)
  apply(simp_all add:own\<^sub>W_n_by_t_imp_def main_inv_lemmas general_structure_lemmas)
  apply(simp_all add:allocated_addresses_lemmas Rcap_def Wcap_def)
  apply clarsimp
  apply clarsimp
  apply clarsimp
  apply metis
  ...
  apply(simp_all add:allocated_addresses_lemmas Rcap_def Wcap_def)
  apply clarsimp
  apply(case_tac "\<not>snd (CAS t (a, b) (the (s ms t)) (the (n ms t)) \<sigma> ts')", simp_all)
  apply(simp add:observation_inv_sig_def covered_v_def visible_writes_def)
  unfolding lastWr_def apply clarsimp
  apply(simp add:allocated_addresses_lemmas Rcap_def Wcap_def)
  apply(simp add:observation_inv_sig_def covered_v_def visible_writes_def)
  unfolding lastWr_def apply clarsimp
  apply(simp add:allocated_addresses_lemmas Rcap_def Wcap_def) 
  apply clarify apply(intro conjI impI)
  apply metis apply clarsimp
  apply(subgoal_tac "the(n ms t) \<notin> detaddrs ms t") 
  apply (metis detaddrs_def inlist_def mem_Collect_eq) prefer 2
  apply(subgoal_tac "the(n ms t) \<notin> detaddrs ms t") 
  sledgehammer
